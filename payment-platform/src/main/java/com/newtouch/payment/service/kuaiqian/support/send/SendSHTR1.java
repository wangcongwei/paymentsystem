...
public class SendSHTR1  {
	
	//初始化配置文件
	private static Properties p=PropFile.getProps("/payment/quickpay/mgw.properties");
	
	private static Log logger = LogFactory.getLog(SendSHTR1.class);
	/**
	...
	public static StringBuffer transParams(StringBuffer reqXml,String paraName,String paraValue ,TransInfo transInfo){
		...
		if(reqXml==null){
			...
			reqXml.append(contentXML).append("<").append(transInfo.getRecordeText_1()).append(">")
				  ...
				  .append("<merchantId>").append(p.getProperty("merchantIdSH")).append("</merchantId>")
//				  .append("<settleMerchantId>").append(p.getProperty("settleMerchantId")).append("</settleMerchantId>")
				  .append("<terminalId>").append(p.getProperty("terminalIdSH")).append("</terminalId>")
	...
	public static HashMap sendTR1(String tr1XML , TransInfo transInfo){
		...
		try {
			...
			merchantInfo.setCertPass(p.getProperty("certPasswordSH"));
			merchantInfo.setCertPath(p.getProperty("certFileName"));
			merchantInfo.setMerchantId(p.getProperty("merchantIdSH"));
			merchantInfo.setPassword(p.getProperty("merchantLoginKeySH"));
			merchantInfo.setTimeOut(p.getProperty("timeout"));
			merchantInfo.setDomainName(p.getProperty("domainNameSH"));
			merchantInfo.setSslPort(p.getProperty("sslPortSH"));
		...
		try {
			...
			if(is!=null){
				...
				logger.info("[receive message is ] " + reqData);
